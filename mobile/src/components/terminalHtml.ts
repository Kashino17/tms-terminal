...
(function() {
  ...
  var SQL_PLAIN_RE = /((?:SELECT|INSERT\\s+INTO?|UPDATE\\s+\\S+\\s+SET|DELETE\\s+FROM|CREATE\\s+(?:TABLE|OR\\s+REPLACE\\s+VIEW|INDEX|VIEW|DATABASE|SCHEMA)|DROP\\s+(?:TABLE|INDEX|VIEW|DATABASE)|ALTER\\s+TABLE)\\b[\\s\\S]*?;)/gim;
